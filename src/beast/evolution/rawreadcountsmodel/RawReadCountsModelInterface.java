...
public interface RawReadCountsModelInterface {
    ...
    abstract class Base extends CalculationNode implements RawReadCountsModelInterface {
        ...
        public final Input<Boolean> singleADOInput = new Input<>(
                "singleADO",
                "single ADO mode; the default is single ADO (true); set false to have locus dropout. " +
                        "Note that the definition of the \"adoRate\" parameter is different: for single ADO mode, " +
                        "it is the probability of an ADO happening to a cell at a site; for locus dropout mode, " +
                        "it is the probability of an ADO happening to an allele, where different alleles are " +
         ...
         * @param taxonIndex    apparently
         * @param genotypeIndex apparently
         * @param MLCompIndex   index to the maximum likelihood component corresponding to the number of sequenced alleles
         * @return the mixed likelihood
         */
        public abstract double computeMixedLikelihood(
                final int matrixIndex,
        ...
        /**
         * Allocate an array of doubles whose length is of the modelled number of alleles.
         * @return apparently
         */
        protected double[] getAllocatedArr() {
            double[] ret = new double[modeledAllelesSize];

            if (useLogPartials) {
                Arrays.fill(ret, -Double.MAX_VALUE);
            }
