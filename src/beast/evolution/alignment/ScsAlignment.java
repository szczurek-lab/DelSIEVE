...
public class ScsAlignment extends Map<String> {
    ...
    final public Input<Long> bgSitesNumInput = new Input<>("bgSitesNum",
            "the manipulated number of background sites only for acquisition bias correction, not for " +
                    ...
                    "not enough for acquisition bias correction. This is mutually exclusive with " +
                    "'bgSites2VarSitesRatio'.");

    final public Input<Double> bgSites2VarSitesRatioInput = new Input<>("bgSites2VarSitesRatio",
            "the ratio between the number of background sites and the number of variant sites. The " +
                    "manipulated number of background sites is computed accordingly and only for acquisition bias " +
                    "correction, not for background likelihood. This number should be set only when the number of " +
                    "background sites is not enough for acquisition bias correction. This is mutually exclusive " +
                    "with 'bgSitesNumInput'.");
    ...
    private void initializeBackgroundInfo() {
        ...
        if (bgSitesNumInput.get() != null || bgSites2VarSitesRatioInput.get() != null) {
            if (ascBiasCorrectionInput.get() != ascBiasCorrection.felsenstein)
                throw new IllegalArgumentException("Error! 'bgSitesNum' or 'bgSites2VarSitesRatio' must only be " +
                        "specified when using Felsenstein's bias correction.");

            if (bgSitesNumInput.get() != null)
                manipulatedNrOfBackgroundSites = bgSitesNumInput.get();
            else
                manipulatedNrOfBackgroundSites = (long) (lociNr * bgSites2VarSitesRatioInput.get());

            if (manipulatedNrOfBackgroundSites < nrOfBackgroundSites) {
//                Log.info.println("\nWarning! The manipulated number of background sites (" +
//                        manipulatedNrOfBackgroundSites + ") is less than the number of background sites " +
//                        "inferred from the data (" + nrOfBackgroundSites + "). We will use the later for acquisition " +
//                        "bias correction.");
//
//                manipulatedNrOfBackgroundSites = nrOfBackgroundSites;
                ...
                Log.info.println("\nWarning! The manipulated number of background sites (" +
                        manipulatedNrOfBackgroundSites + ") is less than the number of background sites " +
                        "inferred from the data (" + nrOfBackgroundSites + "). Please note that this will result " +
                        "in biased branch lengths.");
            }

            if (manipulatedNrOfBackgroundSites != nrOfBackgroundSites) {
                Log.info.println("\nThe manipulated number of background sites for acquisition bias correction is " +
                        manipulatedNrOfBackgroundSites + " (originally " + nrOfBackgroundSites + ").\n");
            } else
                Log.info.println("\nThe number of background sites is " + manipulatedNrOfBackgroundSites + ".\n");
        }
    } // initializeBackgroundInfo
    ...
    private void sanityCheckBackgroundInfo() {
        ...
        if (bgSitesNumInput.get() != null && bgSites2VarSitesRatioInput.get() != null) {
            throw new IllegalArgumentException("Error! \"bgSitesNum\" and \"bgSites2VarSitesRatio\" cannot be " +
